...
- `add-frontend-pr-validation`（2026-08-17）：已完成並封存，由 PR #9 合併到 `main`（merge commit `4d9190a`，PR head `c9a057b`）。對應 Issue #4，`.github/workflows/test.yml` 的 `pull_request` trigger 移除 path filter，並新增在 `ubuntu-22.04` 執行的 Node validation job：`npm ci`、`npm run build`、`npm run lint`、`npm run check:i18n` 與 `node --test scripts/*.test.mjs` 全部為不抑制失敗的阻擋步驟；macOS／Windows Rust tests 與 Linux `cargo check` 保持不變。新增 `scripts/check-pull-request-validation.mjs` 與其測試、`npm run check:pull-request-validation`，以及 spec `openspec/specs/pull-request-validation/spec.md`，在 unrestricted trigger、任一必要 Node 指令或既有 Rust 平台覆蓋被移除時失敗。
- 同一個 PR 另含 commit `c9a057b`：Node validation 第一次在 Linux 上執行全部 repository Node contracts 時，`scripts/check-personal-installation.test.mjs` 有 17 個案例失敗。根因是 `checkPersonalInstallation` 透過 macOS-only 的 `plutil` 讀取 `.app` 的 `Info.plist`，在 Linux 上 bundle 一律讀成不存在，與 fixture 內容無關。修正把 18 個會跑完整 checker 的案例改成 `process.platform !== "darwin"` 時 skip（第 18 個原本只是因 `plutil` 缺席而碰巧通過）。後果是這 18 個打包檢查案例目前只在 macOS 主機上執行，而 CI 沒有 macOS Node job，因此它們不再受 CI 把關；要恢復把關必須另外新增 macOS 的 Node validation job。`unsupported_host`、README 文件契約與 evidence 檢查不依賴 macOS，繼續在 Linux 上執行。PR #9 最終 CI 四個 check 全綠：Node validation（223 tests、205 pass、0 fail、18 skipped）、Rust tests macOS、Rust tests Windows、Rust check (Linux)。
